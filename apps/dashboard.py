...
DATA_PATH = Path(__file__).resolve().parents[1] / 'sales_data.csv'
...
# Precompute derived columns so callbacks do not depend on each other
data['Revenues_Difference'] = data['Revenues'].diff().fillna(0)
data['pct_Difference'] = (data['Revenues'].pct_change()) * 100
data['Profit'] = data['Revenues'] - data['Total Cost (Sales & Marketing)']
data['Profit_Difference'] = data['Profit'].diff().fillna(0)
data['Profit_pct_Difference'] = (data['Profit'].pct_change()) * 100
data['Orders_Difference'] = data['Orders Placed'].diff().fillna(0)
data['Orders_pct_Difference'] = (data['Orders Placed'].pct_change()) * 100
data['Customers_Difference'] = data['Customers'].diff().fillna(0)
data['Customers_pct_Difference'] = (data['Customers'].pct_change()) * 100
data['Items_Difference'] = data['Purchased Items'].diff().fillna(0)
...
data['Conversion_Difference'] = data['Conversion Rate'].diff().fillna(0)
...
MONTHS = list(data['Months'].unique())
MONTH_LABELS = {
    'JAN': 'January',
    'FEB': 'February',
    'MAR': 'March',
...
DEFAULT_MONTH = 'MAR' if 'MAR' in MONTHS else MONTHS[0]
        ...
        html.Div([
            ...
            html.Div([
                ...
                dcc.Dropdown(
                    ...
                    searchable=True,
                    value=DEFAULT_MONTH,
                    placeholder='Search or select a month',
                    options=[{'label': MONTH_LABELS.get(c, c), 'value': c}
                             for c in MONTHS],
            ...
            ], className='month_selector_row'),
            dcc.Slider(
                id='month_slider',
                min=0,
                max=len(MONTHS) - 1,
                step=1,
                value=MONTHS.index(DEFAULT_MONTH),
                marks={i: m for i, m in enumerate(MONTHS)},
                included=False,
                tooltip={'placement': 'bottom', 'always_visible': False},
                className='month_slider',
            ),
...
@app.callback(
    ...
    [Input('prev_month', 'n_clicks'),
     Input('next_month', 'n_clicks'),
     Input('select_month', 'value'),
     Input('month_slider', 'value')],
    prevent_initial_call=True
)
def sync_month_controls(prev_clicks, next_clicks, selected_month, slider_index):
    triggered = callback_context.triggered_id
    if triggered in ('prev_month', 'next_month'):
        idx = MONTHS.index(selected_month) if selected_month in MONTHS else 0
        idx = (idx - 1) % len(MONTHS) if triggered == 'prev_month' else (idx + 1) % len(MONTHS)
        return MONTHS[idx], idx
    if triggered == 'month_slider':
        idx = int(slider_index)
        return MONTHS[idx], idx
    if triggered == 'select_month' and selected_month in MONTHS:
        return selected_month, MONTHS.index(selected_month)
    raise PreventUpdate
...
def update_graph(select_month):
    if select_month is None:
        ...
        remaining_percentage_profit = 100 - percentage_profit
...
def update_graph(select_month):
    if select_month is None:
        ...
        remaining_percentage_target = 100 - percentage_target
        colors = ['#DEB340', '#335476']
